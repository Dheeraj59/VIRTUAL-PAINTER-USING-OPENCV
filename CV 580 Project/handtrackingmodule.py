...
class handDetector():
    ...
    def findHands(self, img, draw=True):
        imgRGB = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # converting to RGB because hand recognition works only on RGB image
...
def main():
    ...
    while True:
        ...
        CTime = time.time()
        fps = 1/(CTime-PTime)
        PTime = CTime # here, previous time is replaced by current time
